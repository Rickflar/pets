  ...
  render() {
    ...
    const removeMeetMember = e => {
      ...
      this.api.RemoveMeetMember({
        meet: meet.id
      }).then(() => { this.setState({ disabled: false, ismember: !this.state.ismember }); onStoryChange(activeStory, 'meets') })
    }
    const menu = e => {
      setParentState({
        popout:
            ...
            <Group header={<Header mode="secondary">Действия</Header>}>
              <Div style={{ display: 'flex' }}>
                <Button size="l" disabled={disabled} stretched mode='outline' style={{ marginRight: 12 }}
                  onClick={() => !this.state.ismember ? paticipate() : removeMeetMember()}>{disabled ? <Spinner size='small' /> : !this.state.ismember ? 'Участвовать' : 'Отказаться от участия'}</Button>
