...
import { Panel, PanelHeader, FormLayout, FormStatus, Textarea, Input, Link, Spinner, Group, Header, Radio, File, Checkbox, Button } from '@vkontakte/vkui';
      ...
      <Panel id={id}>
        ...
        <Group header={<Header mode="secondary">Информация о петиции</Header>}>
          <FormLayout>
            ...
            <Checkbox style={{ marginTop: -12 }} checked={state.accept} onChange={(e) => this.props.setParentState({ accept: e.currentTarget.checked })}
            ...
            <Button mode="outline" size="xl" disabled={!this.checkIfAllOk(name, description, start, finish, photo, accept)} style={{ marginTop: -12 }} onClick={() => {
              this.AddMeet();
            }}>
